...
import { curveDenotationForGeometry, isCurveGeometry } from "./curveDenotation";
...
describe("geometry/curveDenotation", () => {
  ...
  test("denotes evaluated lines as unbounded linear curves", () => {
    const line: EvaluatedLine = {
      kind: "LINE",
      sourceKind: "LINE",
      id: "L",
      a: vec2(0, 0),
    ...
    expect(curveDenotationForGeometry(line)).toEqual(
      lineCurveThroughPoints(vec2(0, 0), vec2(1, 0)),
    );
    expect(isCurveGeometry(line)).toBe(true);
  });
