...
export function panViewport(viewState: ViewState, deltaWorld: Vec2): ViewState {
...
export function zoomViewport(viewState: ViewState, factor: number): ViewState {
  if (factor <= 0) {
    throw new Error(`Viewport zoom factor must be positive: ${factor}`);
  }

  return setViewportZoom(viewState, clampZoom(viewState.viewportZoom * factor));
}
...
export function rotateViewportCounterclockwise(
  viewState: ViewState,
): ViewState {
  return rotateViewport(viewState, VIEWPORT_ROTATION_STEP);
}
