...
core must not depend on DOM, canvas, CSS, or browser APIs
```
...
Core owns geometry meaning, graph representation, graph edits, construction helpers, dependency inspection, delete policy, evaluation, diagnostics, workspace serialization, headless viewport math, constrained construction semantics, and the public core API.

It is published internally as `@euclid-forge/core`.

### `apps/forge`

Forge owns the browser/editor shell: canvas rendering, DOM events, commands, pointer intent, modal tools, selection and hover behavior, lasso selection, history, workspace file actions, print/display presentation, smoke tests, and styling.

Forge imports core through `@euclid-forge/core` package paths rather than relative paths into `packages/core`.

## Current product posture

The app supports both power-user keyboard workflows and modal browser/mobile-friendly workflows.
...
- create finite parallel segments using a constrained endpoint
...
- view parallel-family chevrons derived from parallel constructions
