...
describe("representation/constructible point inputs", () => {
  ...
  test("creates a triangle from mixed free and derived vertices", () => {
    expect(triangleConstruction(graphWithDerivedPoints(), ["A", "M", "G"])).toEqual([
      expect.objectContaining({
        kind: "TRIANGLE",
        a: "A",
        b: "M",
  ...
  test("still rejects non-point construction inputs", () => {
    expect(() => segmentConstruction(graphWithDerivedPoints(), "AB", "G")).toThrow(
      "Cannot create segment with constrained endpoint: AB",
    );
    expect(() => circleConstruction(graphWithDerivedPoints(), "ABC", "G")).toThrow(
      "Cannot create circle with constrained point: ABC",
    );
    expect(() => triangleConstruction(graphWithDerivedPoints(), ["A", "M", "AB"])).toThrow(
      "Cannot create triangle with constrained vertex: AB",
    );
  });
});
